...
LOADER_CONFIG = {
    "data_path":           os.path.join(_ROOT, "preprocessing-dataset", "processed_data.csv"),
    "sample_size":         5000,
    "test_size":           0.2,
    "random_state":        42,
    ...
    "rf_estimators":       100,
    "mlp_hidden":          (100,),
    "vqc_weights_path":    os.path.join(_ROOT, "model_VQC",  "result", "vqc_weights.npy"),
    "qsvm_kernel_path":    os.path.join(_ROOT, "model_qsvm", "result", "kernel_matrix_train.npy"),
    # Labels saved by generate_qsvm_labels.py — must match the kernel matrix rows.
    "qsvm_labels_path":    os.path.join(_ROOT, "model_qsvm", "result", "y_qsvm_train.npy"),
}
...
def predict_all(url: str, include_quantum: bool = True) -> dict[str, Any]:
    ...
    combined = np.hstack([svd_feat, [url_feats]])
    ...
    if include_quantum:
        ...
        q_combined = np.hstack([svd_q_feat, [url_feats]])
