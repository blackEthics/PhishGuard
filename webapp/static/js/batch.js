...
(function () {
  ...
  function downloadCSV(results) {
    const header = "index,url,verdict,avg_confidence_pct,phishing_votes,total_models\n";
    const rows = results.map((r, i) =>
      `${i + 1},"${r.url.replace(/"/g, '""')}",${r.verdict},${(r.confidence * 100).toFixed(1)},${r.phishing_votes},${r.total_models}`
    ).join("\n");
